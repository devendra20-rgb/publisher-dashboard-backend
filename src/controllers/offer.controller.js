...
async function list(req, res) {
  try {
    return successResponse(
      ...
      paginated(await getOffers(req.user, req.query)),
    );
  } catch (error) {
...
async function details(req, res) {
  try {
    return successResponse(
      ...
      await getOffer(req.user, req.params.id),
    );
  } catch (error) {
    return errorResponse(res, error.statusCode || 500, error.message, []);
  }
...
async function create(req, res) {
  try {
    return createdResponse(
      ...
      await addOffer(req.user, req.body),
    );
  } catch (error) {
...
async function update(req, res) {
  try {
    return successResponse(
      ...
      await editOffer(req.user, req.params.id, req.body),
    );
  } catch (error) {
    return errorResponse(res, error.statusCode || 400, error.message, []);
  }
...
async function remove(req, res) {
  try {
    await removeOffer(req.user, req.params.id);
...
async function payment(req, res) {
  try {
    return successResponse(
      ...
      await editPaymentDetails(req.user, req.params.id, req.body),
    );
  } catch (error) {
    return errorResponse(res, error.statusCode || 400, error.message, []);
  }
